...
namespace MiSeaCore
{
    public class MiSeaCoreSubsystemTerrain : SubsystemTerrain, IDrawable, IUpdateable
    {
        public override void Load(ValuesDictionary valuesDictionary)
        {
            ...
            else
            {
                ...
                {
                    TerrainContentsGenerator = new 新的地形(this);//就是生成下面的地形，就是这么简单
                }
            }
        }
    ...
    public class 新的地形 : ITerrainContentsGenerator
    {
        ...
        public 新的地形(SubsystemTerrain subsystemTerrain)
        {
        ...
        public void GenerateOreMountains(TerrainChunk chunk)
        {
            ...
            var random = new Random(m_seed + chunk.Coords.X * 19 + chunk.Coords.Y * 47);

            // 降低生成概率到1%
            if (random.Bool(0.01f))
            {
                // 确定矿石山位置（在当前区块内）
                int oreMountainX = chunk.Origin.X + random.Int(4, 11);
                int oreMountainZ = chunk.Origin.Y + random.Int(4, 11);

                // 计算与海岸的距离
                float shoreDistance = CalculateOceanShoreDistance(oreMountainX, oreMountainZ);

                // 只在远离主岛的地方生成矿石山
                if (shoreDistance > 20f)
                {
                    // 获取地面高度
                    int baseHeight = m_subsystemTerrain.Terrain.CalculateTopmostCellHeight(oreMountainX, oreMountainZ);

                    // 检查地面是否为石头
                    int groundBlock = m_subsystemTerrain.Terrain.GetCellContentsFast(oreMountainX, baseHeight, oreMountainZ);
                    bool isStoneGround = (groundBlock == BlocksManager.GetBlockIndex<GraniteBlock>()); 

                    // 确保地面高度合理（在海平面以上）且地面是石头
                    if (baseHeight > OceanLevel + 5 && baseHeight < 200 && isStoneGround)
                    {
                        // 生成不规则三角锥形矿石山
                        GenerateOreMountainShape(chunk, oreMountainX - chunk.Origin.X, oreMountainZ - chunk.Origin.Y, baseHeight, random);
                    }
                }
            }
        }

        private void GenerateOreMountainShape(TerrainChunk chunk, int localX, int localZ, int baseHeight, Random random)
        {
            // 矿石山的最大半径和高度
            int maxRadius = random.Int(2, 4);
            int maxHeight = random.Int(6, 12);

            // 创建不规则三角锥形
            for (int y = 0; y < maxHeight; y++)
            {
                // 随着高度增加，半径逐渐减小，形成锥形
                float heightFactor = 1f - (float)y / maxHeight;
                int radiusAtHeight = (int)(maxRadius * heightFactor);

                // 添加一些不规则性
                radiusAtHeight += random.Int(-1, 1);
                radiusAtHeight = MathUtils.Max(radiusAtHeight, 0);

                if (radiusAtHeight == 0 && y > maxHeight / 2) break; // 顶部变尖

                for (int x = localX - radiusAtHeight; x <= localX + radiusAtHeight; x++)
                {
                    for (int z = localZ - radiusAtHeight; z <= localZ + radiusAtHeight; z++)
                    {
                        // 检查坐标是否在区块范围内
                        if (x >= 0 && x < 16 && z >= 0 && z < 16)
                        {
                            // 计算距离中心的距离，用于形成圆形锥体
                            float distance = Vector2.Distance(new Vector2(x, z), new Vector2(localX, localZ));

                            // 添加一些不规则性
                            float irregularity = random.Float(-0.3f, 0.3f);
                            if (distance <= radiusAtHeight + irregularity)
                            {
                                int currentY = baseHeight + y;
                                if (currentY < 255)
                                {
                                    // 根据高度和随机因素决定矿石类型
                                    float oreHeightFactor = (float)y / maxHeight;
                                    float oreChance = random.Float(0f, 1f);

                                    int blockType = BlocksManager.GetBlockIndex<CobblestoneBlock>(); // 默认石头

                                    if (oreHeightFactor > 0.8f && oreChance < 0.25f)
                                    {
                                        // 顶部钻石矿
                                        blockType = BlocksManager.GetBlockIndex<DiamondOreBlock>();
                                    }
                                    else if (oreHeightFactor > 0.6f && oreChance < 0.4f)
                                    {
                                        // 上部铜矿
                                        blockType = BlocksManager.GetBlockIndex<CopperOreBlock>();
                                    }
                                    else if (oreHeightFactor > 0.3f && oreChance < 0.5f)
                                    {
                                        // 中部铁矿
                                        blockType = BlocksManager.GetBlockIndex<IronOreBlock>();
                                    }
                                    else if (oreChance < 0.3f)
                                    {
                                        // 底部煤矿
                                        blockType = BlocksManager.GetBlockIndex<CoalOreBlock>();
                                    }

                                    chunk.SetCellValueFast(x, currentY, z, Terrain.MakeBlockValue(blockType));
                                }
                            }
                        }
                    }
